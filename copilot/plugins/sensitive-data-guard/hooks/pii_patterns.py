"""Generated distribution copy; source: shared/copilot/pii_patterns.py."""
...
def find_pii_kinds(text: str) -> list:
    """回傳命中的個資種類名稱（去重保序）。

    deny-only 的偵測不需要遮罩結果，只需要知道命中了哪些種類；遮罩函式仍保留在
    RULES 中，以維持與 Claude／Codex 的規則契約一致（供三平台行為一致性測試比對）。
    """
    kinds = []
