...
（`strict`／`standard`／`light`）核准模式。`ORCHESTRATOR.md` 現為精簡治理政策，
只規範授權、外部副作用、驗收、成本與失敗揭露；一般分解、模型選擇及代理調度交由
平台與模型自行決定。
...
| 層次 | 解決的問題 | 實作來源 |
| --- | --- | --- |
| 治理層 | 人類授權、外部副作用、驗收、成本與失敗揭露 | `ORCHESTRATOR.md`、政策檔 |
...
關卡」：治理文件定義不得跨越的邊界，hooks 決定尚未滿足條件時不能做什麼。適合多人
...
| `harness` | 無 | 有 | 有 | 無；歷史提示稿已淘汰 | 為既有流程補硬性防線 |
| `integrated-harness` | 有 | strict 模式有 | 有 | 精簡治理政策 | 高風險治理與後續維護 |
...
`light` 只檢查「拆解文件是否存在且含必要標記」，**不解析也不強制 `## 允許修改範圍`**：
...
| 模型與代理限制 | 禁止使用的模型／供應商、資料落地或區域限制 | 一般路由交由平台；只保留人類必須指定的限制 |
