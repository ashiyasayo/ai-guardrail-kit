...
一般任務分解、模型選擇與代理調度交由目前平台自行決定，本政策不綁定模型名稱或
能力層級。若團隊有禁止使用的模型、資料落地區域或供應商限制，請在此逐項列出；
留空代表沒有額外限制，但不會放寬下方成本與環境授權。

- 禁止使用的模型／供應商：<由人類設定；留空即無額外限制>
- 資料落地或區域限制：<由人類設定；留空即無額外限制>
